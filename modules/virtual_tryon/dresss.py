...
class ShoulderHeightDressWarper:
    ...
    def __init__(self, seg_model_path, saree_model_path=None):
        ...
        if saree_model_path is None:
             # Use Fooocus models directory
             import os
             base_dir = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
             self.saree_model_path = os.path.join(base_dir, 'models', 'segformer-b3-fashion')
        else:
    ...
    def _get_pose_rotation_correction(self, pose):
        """
        Estimates a correction factor to recover 'true' body width 
        based on the person's rotation (3/4 angle).
        Uses a conservative baseline to reach a "Perfect" fit.
        """
        if not pose:
            return 1.0
            
        required_keys = ["l_shoulder", "r_shoulder", "l_hip", "r_hip"]
        if not all(k in pose and pose[k] is not None for k in required_keys):
            return 1.0
            
        # 1. Body width (Max of Shoulders and Hips for robustness)
        s_w = np.linalg.norm(pose["l_shoulder"] - pose["r_shoulder"])
        h_w = np.linalg.norm(pose["l_hip"] - pose["r_hip"])
        obs_max_w = max(s_w, h_w)
        
        # 2. Torso height estimate
        s_y = (pose["l_shoulder"][1] + pose["r_shoulder"][1]) / 2.0
        h_y = (pose["l_hip"][1] + pose["r_hip"][1]) / 2.0
        torso_h = abs(h_y - s_y) + 1e-6
        
        # 3. Observed ratio (Max Width / Torso Height)
        ratio = obs_max_w / torso_h
        
        # Baseline for a frontal person (Max Width to Torso Height)
        # Reduced from previously high estimates to a Perfect baseline of 1.10
        baseline = 1.10
        
        if ratio < baseline:
            # Correction factor to "re-widen" the body to the frontal equivalent.
            factor = baseline / ratio
    ...
    def normalize_and_scale(self, dress, c_pose, p_pose, mask, person_shape):
        ...
        # Strict detection: ensure landmarks are within image and have good volume
        has_torso = False
        if c_pose:
            required_keys = ["l_shoulder", "r_shoulder", "l_hip", "r_hip"]
            if all(k in c_pose and c_pose[k] is not None for k in required_keys):
                # Check for zero-width or degenerate poses
                src_s_w = np.linalg.norm(c_pose["l_shoulder"] - c_pose["r_shoulder"])
                src_h_w = np.linalg.norm(c_pose["l_hip"] - c_pose["r_hip"])
                if src_s_w > 5 and src_h_w > 5:
                    has_torso = True
        ...
        if has_torso:
            ...
            # Apply Anatomical Fit Scaling Correction
            corr_src = self._get_pose_rotation_correction(c_pose)
            corr_dst = self._get_pose_rotation_correction(p_pose)
            
            # Calculate "Virtual Frontal Widths" using MAX of (Shoulders, Hips)
            # This ensures the garment always reaches the widest part of the torso
            src_s_w = np.linalg.norm(c_pose["l_shoulder"] - c_pose["r_shoulder"])
            src_h_w = np.linalg.norm(c_pose["l_hip"] - c_pose["r_hip"])
            src_max_w = max(src_s_w, src_h_w) * corr_src
            
            dst_s_w = np.linalg.norm(p_pose["l_shoulder"] - p_pose["r_shoulder"])
            dst_h_w = np.linalg.norm(p_pose["l_hip"] - p_pose["r_hip"])
            dst_max_w = max(dst_s_w, dst_h_w) * corr_dst
            
            # Horizontal Scale 
            sx = dst_max_w / (src_max_w + 1e-6)
            sx = np.clip(sx, 0.4, 2.8)

            # Height scaling using hips/knees/ankles (Vertical scaling is less affected by rotation)
    ...
    def warp(self, dress, c_pose, p_pose, person_shape):
        ...
        # Check for Case 1 (person in cloth) alignment
        has_torso = False
        if c_pose:
            required_keys = ["l_shoulder", "r_shoulder", "l_hip", "r_hip"]
            if all(k in c_pose and c_pose[k] is not None for k in required_keys):
        ...
        if has_torso:
            # Re-verify scaling in warp function using the same MAX-Width Anatomical logic
            corr_src = self._get_pose_rotation_correction(c_pose)
            corr_dst = self._get_pose_rotation_correction(p_pose)
            
            src_anchor = (c_pose["l_shoulder"] + c_pose["r_shoulder"]) * 0.5
            
            # Use MAX torso width for perfect coverage
            src_s_w = np.linalg.norm(c_pose["l_shoulder"] - c_pose["r_shoulder"])
            src_h_w = np.linalg.norm(c_pose["l_hip"] - c_pose["r_hip"])
            cloth_max_w = max(src_s_w, src_h_w) * corr_src
            
            dst_s_w = np.linalg.norm(p_pose["l_shoulder"] - p_pose["r_shoulder"])
            dst_h_w = np.linalg.norm(p_pose["l_hip"] - p_pose["r_hip"])
            person_max_w = max(dst_s_w, dst_h_w) * corr_dst

            if cloth_max_w > 10:
                scale_x = person_max_w / (cloth_max_w + 1e-6)
                scale_x = np.clip(scale_x, 0.4, 3.0)
            else:
                scale_x = 1.0
        else:
