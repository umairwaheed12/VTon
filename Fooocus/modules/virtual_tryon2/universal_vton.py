    ...
    def process(self, person_img, cloth_img, out_dir, original_img=None, clean_img=None):
        ...
        if clean_img is not None:
            ...
            try:
                ...
                # Use provided B2 model path
                print(f"Initializing Cloth Remover (universal mode) from: {self.b2_model_path if hasattr(self, 'b2_model_path') else 'B2'}")
                remover = ClothRemover(str(self.b2_session.get_inputs()[0].name) if False else None) # Dummy, session exists
                # Re-init remover with correct path
                remover = ClothRemover(str(self.base_dir / "models" / "SegFormerB2Clothes" / "segformer_b2_clothes.onnx") if not Path(self.base_dir / "models" / "SegFormerB2Clothes" / "segformer_b2_clothes.onnx").exists() else str(self.base_dir / "models" / "segformer_b2_clothes.onnx"))
                # Actually, I should just use the path passed to __init__ if I store it, but I didn't.
                # Let's just use the logic from other files.
                remover = ClothRemover(str(self.base_dir / "models" / "SegFormerB2Clothes" / "segformer_b2_clothes.onnx"))
                if not Path(remover.seg_model_path).exists():
                    remover = ClothRemover(str(self.base_dir / "models" / "segformer_b2_clothes.onnx"))
...
def main():
    ...
    parser.add_argument("--person", type=str, default="person.png", help="Path to person image")
    parser.add_argument("--cloth", type=str, default="cloth.png", help="Path to cloth image")
    parser.add_argument("--out", type=str, default="universal_output", help="Output directory")
