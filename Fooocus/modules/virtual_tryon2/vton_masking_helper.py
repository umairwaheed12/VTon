...
class VTONMasker:
    ...
    def get_final_mask(self, original_img, warped_mask, mode='universal', clean_img=None, garment_type=None):
        ...
        # 9. MAXIMUM ABSOLUTE FACE & HAIR PROTECTION (SAM-Backed for Razor Sharpness)
        print("  🎭 VTONMasker: Applying SAM-Backed Absolute Face & Hair Shield...")
        ...
        if hasattr(self, 'sam_predictor') and self.sam_predictor is not None:
             # Sample point on face/hair area (MediaPipe nose as anchor)
             img_rgb = cv2.cvtColor(original_img, cv2.COLOR_BGR2RGB)
             res = self.pose_detector.process(img_rgb)
             if res.pose_landmarks:
                  ...
                  masks, _, _ = self.sam_predictor.predict(input_point, input_label, multimask_output=True)
                  # Take the largest mask which likely covers head/hair
        ...
        if res.pose_landmarks:
             ...
             nose_y = lm[0].y
             face_height = mouth_y - nose_y
             chin_limit_y = int((mouth_y + face_height * 0.05) * h)
